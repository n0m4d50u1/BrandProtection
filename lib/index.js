...
class DomainPrefSuf extends DomainFuzz { 
  constructor(domain) {
    super(domain);
    this.dictionary = [];
  }
  load_dict(file) {
    if (fs.existsSync(file)) {
      const data = fs.readFileSync(file);
      for (let word of data.toString().split('\n')) {
        word = word.split(' ')[word.toString().split(' ').length-1];
        if (word && !word.startsWith('//')) {
          this.dictionary.push(word);
        }
      }
  ...
  __dirtionary() {
    ...
    for (const word of this.dictionary) {
      if (word[0]=='+'){
        result.push(`${word.substring(1)}-${prefix + name}`);
        result.push(word.substring(1) + prefix + name);
      } else {
        result.push(`${prefix + name}-${word.slice(0,-1)}`);
        result.push(prefix + name + word.slice(0,-1));
      }
    }
    return result;
  ...
  generate() {
    for (const domain of this.__dirtionary()) {
      this.domains.push({ fuzzer: 'Prefix/Suffix', 'domain-name': `${domain}.${this.tld}` });
    }
  }
}
...
exports.DomainPrefSuf = DomainPrefSuf;
